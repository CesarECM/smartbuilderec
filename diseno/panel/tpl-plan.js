...
function _txRow(t) {
  ...
  const codigos = (t.estandar_codigos || []).map(c =>
    `<span class="tx-ec-tag">${c}</span>`).join(' ');
  ...
  return `<tr>
    ...
    <td style="font-size:13px;color:var(--c-text-2);font-weight:600">${label} ${srcTag} ${codigos}${candidatoHtml}${desc}</td>
