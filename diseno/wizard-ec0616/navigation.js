// ─── wizard-ec0616/navigation.js — Sidebar + navegación EC0616 ───────────────

import { FLUJO_SECCIONES, NAV_A_SECCION, NORMA, NORMA_NOMBRE, PROGRESS_STEPS, ELEMENTOS } from "./config.js";

export function getSidebarHTML() {
  const elemsNav = ELEMENTOS.map(e =>
    `<li id="nav16-${e.id}" class="nav-item disabled" data-prereq="Completa el elemento anterior primero">
      Elem. ${e.num} — ${e.titulo.split(" ").slice(0,3).join(" ")}…
    </li>`
  ).join("\n");
  ...
  return `
    ...
    <div id="sbe-sidebar-status" style="padding:8px 14px;border-bottom:1px solid rgba(255,255,255,0.07);">
      <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;">
        ...
        <span id="sbe-progress-label" style="font-size:10px;color:rgba(255,255,255,.4);white-space:nowrap;">0 / 9</span>
      </div>
    ...
    <nav class="menu">
      <ul class="nav-grupos">
        <li class="nav-grupo">
          <div class="nav-grupo-header" data-grupo="candidato"><span>📋 Candidato</span><span class="nav-grupo-arrow">▾</span></div>
          <ul class="nav-grupo-items open">
            <li id="nav16-datos" class="nav-item active">Datos del candidato</li>
          </ul>
        </li>
        <li class="nav-grupo">
          <div class="nav-grupo-header" data-grupo="elementos"><span>🏥 Elementos EC0616</span><span class="nav-grupo-arrow">▾</span></div>
          <ul class="nav-grupo-items open">
            ${elemsNav}
          </ul>
        </li>
        <li class="nav-grupo">
          <div class="nav-grupo-header" data-grupo="portafolio"><span>📄 Portafolio</span><span class="nav-grupo-arrow">▾</span></div>
          <ul class="nav-grupo-items open">
            <li id="nav16-portafolio" class="nav-item disabled" data-prereq="Completa los 7 elementos primero">Revisión y descarga ZIP</li>
          </ul>
        </li>
      </ul>
    </nav>
...
export function actualizarProgressBar() {
  ...
  if (lbl) lbl.textContent = `${completados} / 9`;
...
export function initNavigation() {
  ...
  window.mostrarSeccion16 = mostrarSeccion;
  window.desbloquear16    = desbloquear;
}
