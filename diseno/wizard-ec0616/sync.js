// ─── wizard-ec0616/sync.js — Sincronización EC0616 ↔ Supabase ────────────────
// Independiente de _sbeCore. Lee ec0616_* de localStorage real y guarda
// en la tabla ec0616_datos. Expone window.ec0616Sync.

(function () {
  "use strict";

  const DEBOUNCE_MS = 2500;
  const TABLA       = "ec0616_datos";
  const ID_KEY      = "sbe_ec0616_id";

  let _timer   = null;
  let _syncing = false;

  function _toast(tipo, msg) {
    let el = document.getElementById("_ec0616-toast");
    if (!el) {
  ...
  function _recolectar() {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (!key || !key.startsWith("ec0616_")) continue;
  ...
  function _restaurar(datos) {
    ...
    Object.entries(datos).forEach(([shortKey, val]) => {
      localStorage.setItem(`ec0616_${shortKey}`, typeof val === "string" ? val : JSON.stringify(val));
    });
  }
  ...
  async function syncToSupabase() {
    ...
    try {
      ...
      if (!estado.datos || !estado.datos.nombre_candidato) { _toast("ok"); return; }

      const nombreCand = [estado.datos.nombre_candidato, estado.datos.apellidos_candidato].filter(Boolean).join(" ");
  ...
  async function init() {
    ...
    const targetId = params.get("ec0616_id") || localStorage.getItem(ID_KEY);
    if (!targetId) return;
    ...
    const url = new URL(window.location.href);
    url.searchParams.delete("ec0616_id");
    window.history.replaceState({}, "", url.toString());
  }
  ...
  function limpiar() {
    ...
    for (let i = localStorage.length - 1; i >= 0; i--) {
      ...
      if (k && k.startsWith("ec0616_")) localStorage.removeItem(k);
    }
  ...
  window.ec0616Sync = { init, syncNow: syncToSupabase, limpiar, schedule };
})();
